  ...
  return {
    ...
    modifiers: [
      ...
      ...(node.modifiers ?? []).filter(
        (s: any) =>
          !s?.expression?.expression?.text ||
          !names.includes(s.expression.expression.text),
      ),
    ],
  };
}
...
export function createObjectTypeDecorator(context: Context) {
  ...
  return factory.createDecorator(
    factory.createCallExpression(
      factory.createIdentifier("ObjectType"),
      undefined,
      undefined,
    ),
  );
    ...
    if (
      ...
      if (statement.importClause) {
        if (
          ...
          ts.isIdentifier(statement.importClause.name)
        ) {
    ...
    statements: [
      ...Object.keys(importStatements)
        ...
        .map((fromFile) =>
          createImport(
            fromFile,
            ...Array.from(importStatements[fromFile]).sort(),
          ),
        ),
      ...Object.keys(importStatements)
        .filter((fromFile) => !!importStatements[fromFile].size)
